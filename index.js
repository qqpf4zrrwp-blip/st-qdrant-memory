...
async function indexCharacterChats() {
  ...
  // Attach to body. position:fixed + inset:0 then resolves against the
  // viewport. (Do NOT attach to #movingDivs: it is a transformed, near-zero-
  // height drag layer on mobile, which clips a fixed overlay to a thin strip.)
...
async function showMemoryViewer() {
            ...
            ">
                <div style="color: #333; word-break: break-all; overflow-wrap: anywhere;">
                    ...
                    <div style="margin-top: 20px; display: flex; gap: 10px; flex-wrap: wrap;">
                        <button id="qdrant_delete_collection_btn" class="menu_button" style="background-color: #dc3545; color: white; flex: 1 1 140px; min-height: 44px; padding: 10px 16px;">
                            刪除所有記憶
                        </button>
                        <button id="qdrant_close_modal" class="menu_button" style="flex: 1 1 100px; min-height: 44px; padding: 10px 16px;">
                            關閉
                        </button>
                    </div>
                </div>
  ...
  const $ = window.$
  // Attach to body so position:fixed resolves against the viewport.
  // (Not #movingDivs — see note in the index-progress modal.)
  $("body").append(modalHtml)
