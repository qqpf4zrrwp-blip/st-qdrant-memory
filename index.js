...
async function indexCharacterChats() {
  ...
  // Build the progress content as a live element. The native popup owns
  // positioning/sizing (correct on mobile); we just update these nodes.
  const contentEl = htmlToElement(`
    <div style="text-align:left; color: inherit;">
      <h3 style="margin-top: 0;">索引對話 - ${characterName}</h3>
      <p id="qdrant_index_status">正在掃描對話檔案...</p>
      <div style="background: rgba(128,128,128,0.25); border-radius: 5px; height: 20px; margin: 15px 0; overflow: hidden;">
        <div id="qdrant_index_progress" style="background: #4CAF50; height: 100%; width: 0%; transition: width 0.3s;"></div>
      </div>
      <p id="qdrant_index_details" style="font-size: 0.9em; opacity: 0.7;"></p>
    </div>
  `)

  const q = (sel) => contentEl.querySelector(sel)
  const setStatus = (t) => { const n = q("#qdrant_index_status"); if (n) n.textContent = t }
  const setDetails = (t) => { const n = q("#qdrant_index_details"); if (n) n.textContent = t }
  const setProgress = (pct) => { const n = q("#qdrant_index_progress"); if (n) n.style.width = `${pct}%` }

  const api = getPopupApi()
  let popupRef = null
  let usedFallback = false

  // Shared cancellation flag (object so the popup .then can flip it).
  const cancelledRef = { value: false }

  if (api && api.Popup) {
    // Cancel button = the popup's own cancel; OK hidden until done.
    popupRef = new api.Popup(contentEl, api.POPUP_TYPE.TEXT, "", {
      okButton: false,
      cancelButton: "取消",
      wide: true,
      allowVerticalScrolling: true,
    })
    // show() resolves when the user closes it; we don't await here because we
    // need to keep updating progress. We track cancellation via the promise.
    popupRef.show().then((result) => {
      // result is falsy/null when cancelled by the user.
      cancelledRef.value = true
    })
  } else {
    // Fallback: minimal hand-rolled overlay (only if no native popup at all).
    usedFallback = true
    const fb = `
      <div id="qdrant_index_overlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:99999;overflow:auto;-webkit-overflow-scrolling:touch;box-sizing:border-box;padding:5vh 16px;text-align:center;">
        <div id="qdrant_index_modal" style="display:inline-block;text-align:left;background:#fff;color:#333;padding:20px;border-radius:10px;width:100%;max-width:500px;box-sizing:border-box;"></div>
      </div>`
    $("body").append(fb)
    document.getElementById("qdrant_index_modal").appendChild(contentEl)
    const cancelBtn = document.createElement("button")
    cancelBtn.className = "menu_button"
    cancelBtn.style.cssText = "min-height:44px;padding:10px 16px;width:100%;margin-top:15px;"
    cancelBtn.textContent = "取消"
    cancelBtn.id = "qdrant_index_cancel"
    document.getElementById("qdrant_index_modal").appendChild(cancelBtn)
  }
  ...
  const closeModal = () => {
    ...
    if (!popupRef) return
    // Try the available completion methods across ST versions.
    const tryFns = ["completeAffirmative", "complete", "completeCancelled", "hide"]
    for (const fn of tryFns) {
      if (typeof popupRef[fn] === "function") {
        try {
          fn === "complete" ? popupRef.complete(1) : popupRef[fn]()
          return
        } catch (e) {
          /* try next */
        }
  ...
  // When done, turn the cancel affordance into a close affordance.
  const setCancelButtonToClose = () => {
    if (usedFallback) {
      const b = document.getElementById("qdrant_index_cancel")
      if (b) {
        b.disabled = false
        b.textContent = "關閉"
    ...
    // For the native popup, its built-in Cancel button already closes it,
    // and we auto-close shortly after completion below.
  }
  ...
  // Local alias kept so the rest of the function reads unchanged.
  const cancelledGetter = () => cancelledRef.value
  ...
  try {
    ...
    setStatus(`找到 ${chatFiles.length} 個對話檔案`)
    ...
    for (let i = 0; i < chatFiles.length; i++) {
      if (cancelledGetter()) break

      const chatFile = chatFiles[i]
      const progress = ((i / chatFiles.length) * 100).toFixed(0)

      setProgress(progress)
      setStatus(`處理對話 ${i + 1}/${chatFiles.length}`)
      setDetails(`檔案: ${chatFile}`)
      ...
      for (const chunk of chunks) {
        ...
        setDetails(`已儲存: ${savedChunks} | 已跳過: ${skippedChunks} | 總計: ${totalChunks}`)
      }
    }
    ...
    setCancelButtonToClose()
    // Native popup: auto-close a moment after completion so the user isn't
    // stuck (toastr already reports the result).
    if (!usedFallback) {
...
// ----------------------------------------------------------------------------
// Native SillyTavern popup helper.
// The whole app uses this popup system, so it is already correct on mobile.
// Hand-rolled fixed-position overlays were fighting ST's mobile layout
// (collapsing to a thin strip), so we use the native popup instead.
// ----------------------------------------------------------------------------

// Resolve the popup API from the ST context across versions.
function getPopupApi() {
  try {
    const ctx = getContext()
    if (ctx) {
      const Popup = ctx.Popup || (window.SillyTavern && window.SillyTavern.Popup)
      const POPUP_TYPE = ctx.POPUP_TYPE || (window.SillyTavern && window.SillyTavern.POPUP_TYPE)
      const callGenericPopup =
        ctx.callGenericPopup || (window.SillyTavern && window.SillyTavern.callGenericPopup)
      if (Popup && POPUP_TYPE) return { Popup, POPUP_TYPE, callGenericPopup }
      if (callGenericPopup && POPUP_TYPE) return { Popup: null, POPUP_TYPE, callGenericPopup }
    }
  } catch (e) {
    /* fall through */
  }
...
// Build a DOM element from an HTML string (so we can attach live handlers and
// update progress text while the popup is open).
...
async function showMemoryViewer() {
  ...
  // Content element (no positioning/sizing CSS — the native popup owns that).
  const contentHtml = `
    <div style="text-align: left; word-break: break-all; overflow-wrap: anywhere;">
      <h3 style="margin-top: 0;">記憶檢視 - ${characterName}</h3>
      <p><strong>集合名稱:</strong> ${collectionName}</p>
      <p><strong>記憶總數:</strong> ${count}</p>
      <button id="qdrant_delete_collection_btn" class="menu_button" style="background-color: #dc3545; color: white; min-height: 44px; padding: 10px 16px; width: 100%; margin-top: 14px;">
        刪除所有記憶
      </button>
    </div>
  `
  ...
  if (api) {
    const el = htmlToElement(`<div>${contentHtml}</div>`)

    // Wire the delete button before showing.
    const wireDelete = (closeFn) => {
      const btn = el.querySelector("#qdrant_delete_collection_btn")
      if (!btn) return
      btn.addEventListener("click", async () => {
        const confirmed = confirm(`確定要刪除 ${characterName} 的所有記憶嗎？此操作無法復原！`)
        if (!confirmed) return
        btn.disabled = true
        btn.textContent = "刪除中..."
        ...
        if (success) {
          ...
          btn.disabled = false
          btn.textContent = "刪除所有記憶"
        }
      })
    }

    if (api.Popup) {
      const popup = new api.Popup(el, api.POPUP_TYPE.TEXT, "", {
        okButton: "關閉",
        wide: true,
        allowVerticalScrolling: true,
      })
      wireDelete(() => popup.completeAffirmative())
      await popup.show()
    } else {
      wireDelete(null)
      await api.callGenericPopup(el, api.POPUP_TYPE.TEXT, "", {
        okButton: "關閉",
        wide: true,
        allowVerticalScrolling: true,
      })
  ...
  // Last-resort fallback: hand-rolled overlay (only if native popup is
  // somehow unavailable in this ST build).
  const modalHtml = `
    <div id="qdrant_overlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:99999;overflow:auto;-webkit-overflow-scrolling:touch;box-sizing:border-box;padding:5vh 16px;text-align:center;">
      <div id="qdrant_modal" style="display:inline-block;text-align:left;background:#fff;padding:20px;border-radius:10px;width:100%;max-width:500px;box-sizing:border-box;">
        ${contentHtml}
        <button id="qdrant_close_modal" class="menu_button" style="min-height:44px;padding:10px 16px;width:100%;margin-top:10px;">關閉</button>
      </div>
    </div>`
  $("body").append(modalHtml)
  $("#qdrant_modal").on("click", (e) => e.stopPropagation())
  $("#qdrant_close_modal, #qdrant_overlay").on("click", () => $("#qdrant_overlay").remove())
